...
| ③ 전처리 | 노출 보정(선택: 무채색 배경 기반 WB), 배경 마스크, 해상도 조정 | 정규화 이미지 + 마스크 |
...
### 1.3 범위 밖 (후속 단계)
- 모발 1가닥 단위 재구성(굵기·밀도 측정)은 트리코스코피(확대 영상) 영역이다. 3D 모델은 **거시적 분포·면적**을 담당하고, 모발 밀도/굵기는 트리코스코피 결과를 3D 지도 위에 **등록(registration)** 하는 방식으로 결합한다(6장).
...
afs3d/
  ...
  afs3d/
    ...
    preprocess.py       노출 정규화, (선택) 배경 기반 WB, 배경 마스크, 리사이즈(EXIF 보존)
    ...
    server.py           원내 웹 앱 서버(업로드·작업 대기열·진행률·결과 제공, 표준 라이브러리만 사용)
    cli.py              명령행: check / prep / reconstruct / analyze / run / demo / serve
  web/                  웹 앱 화면(업로드 → 진행 → 3D 결과), three.js 동봉
  viewer/index.html     단독 3D 뷰어(드래그&드롭, 회전, 두 점 거리 측정)
  tests/                단위·통합 테스트
```
...
- **화이트밸런스는 기본적으로 하지 않는다**: 머리 사진에 회색세계(gray-world) 가정을 적용하면 피부·모발색 자체를 색 틀어짐으로 보고 지워 피부가 회색이 된다(프로토타입 검증 중 발견해 수정). 카메라 WB 고정이 원칙이며, 배경막이 무채색일 때만 배경 픽셀로 세트 공통 WB 를 잡는 옵션(`--white-balance background`)을 쓴다. 색 재현이 중요하면 컬러체커 촬영을 권장.
...
### 7.1 웹 앱: 사진을 올리면 3D 모델이 만들어지는 방식 (`afs3d serve`)

진료실에서는 명령행 대신 브라우저 화면으로 쓴다.

```mermaid
flowchart LR
  U[브라우저<br/>사진 폴더 끌어다 놓기] -->|사진 1장씩 업로드| S[원내 PC 의 afs3d 서버<br/>127.0.0.1:8765]
  S --> Q[작업 대기열<br/>한 번에 1건]
  Q --> P[afs3d run<br/>전처리→COLMAP→분석]
  P -->|진행 단계| S
  S -->|3D 모델·요약·지도| U
```

- **원내 처리 원칙**: 서버는 기본적으로 그 PC 에서만 접속된다. 원내 다른 PC 에서 쓰려면 `--host 0.0.0.0` → 접속 토큰이 자동 생성되고 토큰이 있는 주소로만 열린다. 사진·결과는 `jobs/<작업ID>/` 에만 저장되고 화면의 **삭제**로 지운다.
- **입력**: 사진 폴더(하위 폴더 포함), 개별 파일, ZIP, `manifest.csv`. 파일명은 경로를 제거하고 허용 확장자만 받는다.
- **처리**: 작업마다 `python -m afs3d run` 을 별도 프로세스로 실행하고, 출력에서 COLMAP 단계를 읽어 진행률로 보여 준다. 서버가 재시작되면 진행 중이던 작업은 "실패(재시작)"로 표시되어 다시 시작할 수 있다.
- **결과 화면**: 3D 보기(탈모 분석 메쉬 / 원본 메쉬 / 점군+카메라), 결과 요약(등록 사진 수, 노출 두피 면적·비율, 이식 밀도별 모낭수), 정수리 지도, 파일 받기(PLY/PNG/JSON/CSV), 처리 기록.
- **GPU 가 없을 때**: 메쉬 단계를 건너뛰고 점군+카메라까지 보여 주며, 그 사실을 화면에 경고로 표시한다.
- 추가 설치 없이 파이썬 표준 라이브러리로 동작하고, 3D 표시 라이브러리(three.js)를 함께 배포해 인터넷 없이도 열린다.
...
| 웹 앱(업로드 → 3D) | 구현 | 서버 API 테스트(업로드·ZIP·경로 조작 차단·토큰·재시작), 브라우저로 합성 100컷 업로드 → COLMAP 처리 → 결과 표시까지 실행 |
